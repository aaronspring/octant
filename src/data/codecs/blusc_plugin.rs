//! Pure Rust Blosc codec plugin for `zarrs` WebAssembly data decoding.
...
impl ExtensionNameStatic for BluscCodec {
    const DEFAULT_NAME_FN: fn(ZarrVersion) -> Option<Cow<'static, str>> =
        |_| Some(Cow::Borrowed(IDENTIFIER));
}

impl ExtensionAliases<ZarrVersion3> for BluscCodec {
...
impl ExtensionAliases<ZarrVersion2> for BluscCodec {
    fn aliases() -> RwLockReadGuard<'static, ExtensionAliasesConfig> {
...
impl CodecTraits for BluscCodec {
...
impl CodecTraitsV3 for BluscCodec {
    fn create(_metadata: &MetadataV3) -> Result<Codec, PluginCreateError> {
        Ok(Codec::BytesToBytes(Arc::new(BluscCodec::new())))
    }
}

impl CodecTraitsV2 for BluscCodec {
    fn create(_metadata: &MetadataV2) -> Result<Codec, PluginCreateError> {
        Ok(Codec::BytesToBytes(Arc::new(BluscCodec::new())))
    }
}

impl BytesToBytesCodecTraits for BluscCodec {
    ...
    fn encode<'a>(
        ...
        let compressed = blusc::convenience::blosc2_compress(&value)
            .map_err(|e| CodecError::Other(format!("Blusc compression failed: {e:?}")))?;
        Ok(compressed.into())
    }
}
...
#[cfg(target_arch = "wasm32")]
inventory::submit! {
    CodecPluginV3::new::<BluscCodec>()
}

#[cfg(target_arch = "wasm32")]
inventory::submit! {
    CodecPluginV2::new::<BluscCodec>()
}
